...
/**
 * REST controller for managing {@link ke.natujenge.alumni.portal.domain.Payment}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Payment> result = paymentRepository
            ...
            .map(existingPayment -> {
                if (payment.getProductId() != null) {
                    existingPayment.setProductId(payment.getProductId());
                }
                if (payment.getCode() != null) {
                    existingPayment.setCode(payment.getCode());
                }
                if (payment.getSubscriberId() != null) {
                    existingPayment.setSubscriberId(payment.getSubscriberId());
                }
                if (payment.getReceiptId() != null) {
                    existingPayment.setReceiptId(payment.getReceiptId());
                }
                ...
                if (payment.getPaybill() != null) {
                    existingPayment.setPaybill(payment.getPaybill());
                }
                if (payment.getTrxId() != null) {
                    existingPayment.setTrxId(payment.getTrxId());
                }
                if (payment.getTrxEndTime() != null) {
                    existingPayment.setTrxEndTime(payment.getTrxEndTime());
                }
                if (payment.getTrxStartTime() != null) {
                    existingPayment.setTrxStartTime(payment.getTrxStartTime());
                }
                if (payment.getStatus() != null) {
                    existingPayment.setStatus(payment.getStatus());
                }
                if (payment.getStatusReason() != null) {
                    existingPayment.setStatusReason(payment.getStatusReason());
                }
                if (payment.getConfigs() != null) {
                    existingPayment.setConfigs(payment.getConfigs());
                }
                if (payment.getCreatedBy() != null) {
                    existingPayment.setCreatedBy(payment.getCreatedBy());
                }
                if (payment.getLastUpdatedOn() != null) {
                    existingPayment.setLastUpdatedOn(payment.getLastUpdatedOn());
                }
                if (payment.getLastUpdtedBy() != null) {
                    existingPayment.setLastUpdtedBy(payment.getLastUpdtedBy());
                }
